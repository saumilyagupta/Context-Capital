...
from context_capital.crypto.signing import (
...
def test_sign_then_verify_roundtrip(
    signing_key: nacl.signing.SigningKey, minimal_doc: dict[str, Any]
) -> None:
...
def test_tampered_memory_fails_verification(
    signing_key: nacl.signing.SigningKey, minimal_doc: dict[str, Any]
) -> None:
    signed = sign_document(minimal_doc, signing_key)
...
def test_sign_refuses_doc_with_existing_signature(
    signing_key: nacl.signing.SigningKey, minimal_doc: dict[str, Any]
) -> None:
    minimal_doc["signature"] = {
        "alg": "ed25519",
        "value": "AA",
        "public_key": "BB",
